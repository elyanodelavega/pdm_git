...
def plot_MPC_det(decisions, t_decision, results, pv_pred, load_pred, figname = None, img_path = None):
    ...
    axes[0].annotate(text='', xy=(0,ypos), xytext=(len(dataset),ypos), arrowprops=dict(arrowstyle='<->',color='black'))
    axes[0].annotate(text='Episode '+str(int(episode)),xy=((len(dataset))/2, ypos+0.2), fontsize=12.0, ha='center')
...
def plot_MPC_sto(decisions, t_decision, results, pv_pred, load_pred, soc_pred, figname = None, img_path = None):
    ...
    x_stop = n-1 + len(predictions)
    low_PV, med_PV, high_PV = confidence_interval(predictions, 0.8, scale = scale_PV)
    axes[0].fill_between(x[n-1:x_stop], low_PV, high_PV, color=palette[2], alpha=0.3, hatch = '//')
    axes[0].plot(x[n-1:x_stop], med_PV, marker='o', color=palette[2], label='Forecast PV')
    
    axes[0].plot(x[n-1:], dataset['load'][n-1:],marker='o', color=palette[3], label='Forecast load')
    ...
    ax2.plot(x[n-1:x_stop], med_SOC, color=palette[7], marker='.')
    ax2.fill_between(x[n-1:x_stop], low_SOC, high_SOC, color=palette[7], alpha = 0.3)
    ax2.set_ylim([0,110])
    #ax2.plot(x[:n], dataset['soc'][:n], color=palette[7], marker='.', label='EV state of charge')
    axes[1].bar(x, dataset['pv_ev'], color=palette[8], edgecolor=palette[8], label='PV supplied to EV')
    axes[1].bar(x, dataset['grid_ev'], color=palette[4], edgecolor=palette[4], label='Grid supplied to EV')
...
#    ax1.grid(False)
    ...
    axes[0].fill_between(x, dataset['pv_real'], color=palette[0], alpha=0.3, label='PV production')
    axes[0].plot(x, dataset['pv_real'], color=palette[0])
    ...
    handles_2, labels_2 = [(a + b) for a, b in zip(axes[1].get_legend_handles_labels(),
                                            ax2.get_legend_handles_labels())]
    axes[0].legend(handles_1, labels_1, loc='lower center', bbox_to_anchor=( 0.5,-0.2),facecolor='white', ncol = 2)
    fig.legend(handles_2, labels_2, loc='lower center', facecolor='white', ncol = 3)
...
def plot_results_comparison(decisions, episodes, figname = None, img_path = None):
    ...
    fig, axes = plt.subplots(len(decisions)+1,1, sharex=True, figsize=(16,9))
