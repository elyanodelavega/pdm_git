...
for e in range_episodes:
    ...
    time_algo['5. MPC stochastic'].append(total_ep_4)
     ...
     #MPC stochastic CVaR
    start_5 = time.time()
    predictions_load['6. MPC stochastic CVaR'][e] = {}
    predictions_PV['6. MPC stochastic CVaR'][e] = {}
    MPC_results['6. MPC stochastic CVaR'][e] = {'results': {}, 'soc': {}}
    model_5 = Model(name = name + methods[2]+str(model_number), data_EV = data_EV, t_start = t_start_episode, 
              t_res = t_res,  EV = EV1, House = House1, spot_prices = spot_prices, V2X = V2X)
    ...
    for t in range(episode_length-1):
        ...
        Load_predictions_5 = Load_model_forecast.predict(t_forecast, t_end)
        PV_predictions_5 = PV_model_forecast.predict_distribution(model = PV_LSTM, time = t_forecast,
                                                            dropout = 0.35, dataframe = True)

        model_5.optimize(t_decision, t_end, PV_predictions_5, Load_predictions_5, forecasting = True, method = 'CVaR', parameters = parameters)
        decisions_5 = model_5.decisions
        results_5 = model_5.results_stochastic()
        MPC_results['6. MPC stochastic CVaR'][e]['results'][t_decision] = results_5
        
        SOC_5 = model_5.predictions_SOC()
        MPC_results['6. MPC stochastic CVaR'][e]['soc'][t_decision] = SOC_5
        
        #plot_MPC(decisions_5, results_5, SOC_5, PV_predictions_5, figname = str(k), img_path = k_path)
        predictions_load['6. MPC stochastic CVaR'][e][t_decision]  = Load_predictions_5
        predictions_PV['6. MPC stochastic CVaR'][e][t_decision] = PV_predictions_5
    
    
    decisions_5 = model_5.decisions[:-1]
    decisions_5 = decisions_5.append(decisions_0.tail(1))
    decisions_5.loc[t_end_episode,'soc'] = model_5.decisions.loc[t_end,'soc']
    
    costs['6. MPC stochastic CVaR'].append(cost)
    results['6. MPC stochastic CVaR'].loc[t_start_episode:t_end_episode] = decisions_5
    
    end_5 = time.time()
    total_ep_5 = end_5 -start_5
    time_algo['6. MPC stochastic CVaR'].append(total_ep_5)
