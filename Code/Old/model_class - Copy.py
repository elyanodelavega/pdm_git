...
class Model:
    ...
    def __init__(self, name, data_full, EV):
    ...
    def get_previous_SOC(self, time):
        
        soc = self.df_prepare.loc[time, 'EV_SOC']
    ...
    def prepare_time(self):
        ...
        T_arrival = list(self.data.index[i+1] for i in range(self.data.shape[0]-1)
                      if (self.EV_Availability[i+1] == 1 and self.EV_Availability[i] ==0))
        
        if self.start == True:
            T_arrival.append(self.time_vector[0] - pd.Timedelta(hours = 1))
        T_arrival.sort()
    ...
    def add_constraints(self):
        ...
        self.SOC_arr = self.m.addConstrs(self.SOC[t] == self.SOC_ini_fixed[t] for t in self.SOC_ini_fixed.keys())
    
    def optimize(self, df_forecasted, start):
        self.start = start
...
#%%
EV = EV_model()
model = Model('model1', data_full = data, EV = EV)
MPC_days = 1/24
t_MPC_start = 560
t_MPC_end = t_MPC_start + MPC_days * 24
start = True
results = []
soc = []
t_arr = []
td = []
for i in range(t_MPC_start, t_MPC_start + 1):
    ...
    df = df_pred(data = data, time = t_forecast, n_hour_future = n_hour_future, 
                 forecast = PV_predictions, t_res = PV_model_forecast.t_res)
    
    model.optimize(df, start = start)
    soc.append(model.SOC_ini_fixed)
    t_arr.append(model.T_arrival)
    td.append(model.T_departure)
    start = False
