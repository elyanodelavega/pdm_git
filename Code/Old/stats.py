...
decisions = {n: pd.read_csv(folder_path+'results_'+n+'_60.csv', index_col = 0) for n in names}
...
for i, s_arr in enumerate(soc_arr):
    for j, n in enumerate(names):
        ...
        for k, e in enumerate(episodes):
            ...
            quantiles = np.quantile(soc,x)
            soc_array[k,:] = quantiles
        # df_pv = pd.DataFrame(data = {int(x[q]*100): soc_array[:,q] for q in range(len(x)-1)})

        # sns.boxplot(data = df_pv, ax = axes[i], orient = 'v', color = color_codes[n],  boxprops=dict(alpha=.5))
