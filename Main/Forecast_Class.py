...
class Forecast_LSTM:
    ...
    def __init__(self, pred_variable, data, n_hour_future, n_hour_past,
                 test_days = 21):
        ''' pred_variable: str, column name of the variable to predict
            data: pandas Dataframe, PV simulation 
            n_hour_future: int, number of hours to predict in the future
            n_hour_past: int, number of past hours used to predict'''
        
        #Initialization
        self.name = pred_variable
        self.data = data
        self.n_hour_future = n_hour_future
        self.n_hour_past = n_hour_past
...
class Forecast_ARMA:
    ...
    def plot_autocorrelation(self):
        ...
        fig, ax = plt.subplots(1,1,figsize = (16,9), dpi = 450)
        pd.plotting.autocorrelation_plot(self.dataset, ax = ax)
        ax.grid(True)
    ...
    def evaluate_arma_model(self, dataset, arma_order):
        ''' Fit an arma model with a particular order (p,q)
        ...
        for t in range(len(test)):
            # Fit
            arma = ARMA(history, order=arma_order)
            model = arma.fit(disp=0)
            # Predict
    ...
    def evaluate_ARMA_models(self, p_values, q_values, plot_results = True):
        ''' Find the best (p,q) order
            input:
                p,d,q values: [int],[int]
            output:
                results: dict'''
        ...
        for p in p_values:
            ...
            for q in q_values:
                ...
                try:
                    s = time.time()
                    mse = self.evaluate_arma_model(dataset, order)
                    e = time.time()
                    results[order] = (mse, e-s)
                    if mse < best_score:
                        best_score, best_conf = mse, order
                    print(f'ARMA{order} MSE={np.round((mse),2)}, time: {np.round((e-s),2)}s')
                except:
                        continue
    
        print(f'Best: ARMA{best_conf} MSE={np.round((best_score),2)}')
        
        self.best_conf = best_conf
        
        orders = results.keys()
        mse = [results[o][0] for o in orders]
        execution_time = [results[o][1] for o in orders]
        
        if plot_results:
            
            self.plot_model_evaluation(orders, mse, execution_time, title = 'Evaluation of (p,q)', xlabel = '$(p,q)$')            
        
            
        return best_conf, best_score
    
    def predict(self, t_forecast, t_end, dataframe = True, best_conf = (2, 1), 
        ...
        if iterative:
            ...
            for t in range(len(forecast_window)):
                # fit
                arma = ARMA(history, order=best_conf)
                model = arma.fit(disp=0)
                # Forecast
    ...
    def predict_with_stats(self, t_forecast, t_end, s = 2):
        ...
        previous = self.data.loc[:t_decision][-s*24:]
    ...
    def evaluate_predict_with_stats(self, S_values, plot_results = True):
        ''' predict with average with previous data, based on hour of the day
        Input:
            S_values: array of int of the test S_values
        Output:
            best_S: int
        '''
        dataset = self.data.copy()
        ...
        # split into days
        dataset['dayofyear'] = [dataset.index[i].dayofyear for i in range(len(dataset))]
        
        days = np.unique(dataset['dayofyear'])[max(S_values)+1:]
        
        # initialize
        MSE_mean = pd.DataFrame(index = days, columns = S_values)
        ...
        for s in S_values:
            ...
            for d in days:
                ...
                df = dataset[dataset.dayofyear == d]
                
                t_forecast = df.index[0]
                
                t_end = df.index[-1]
                
                test = self.data[t_forecast:t_end][self.pred_variable]
                
                forecast_mean = self.predict_with_stats(t_forecast, t_end, s)
                
                MSE_mean.loc[d,s] = mean_squared_error(test, forecast_mean)
                
            end = time.time()
            
            time_vect.append(end-start)
            mse_S = MSE_mean[s].mean()
            print(f'S: {s}, MSE: {mse_S}, time: {np.round(end-start, 4)}')
        ...
        orders = S_values
        mse = list(MSE_mean.mean())
        execution_time = time_vect
        
        if plot_results:
            
            self.plot_model_evaluation(orders, mse, execution_time, title = 'Evaluation of S', xlabel = '$S$')
        
        print(f'Best S: {best_S}, MSE: {best_score}')
        
        return best_S, best_score
    
    def plot_model_evaluation(self, orders, mse, execution_time, title, xlabel):
        # plot evaluation results
        
        fig, ax = plt.subplots(1,1,figsize = (16,9), dpi = 450)
        ax.plot(mse, label = 'MSE', marker = 'o', linewidth = 2)
        ax2 = ax.twinx()
        ax2.set_ylabel('Seconds', fontsize = 18)
        ax2.plot(execution_time, label = 'Training computational time', marker = 'o', color = 'orange', linewidth = 2)
        ax.grid(True)
